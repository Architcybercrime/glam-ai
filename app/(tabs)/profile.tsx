  ...
  return (
    ...
    >
      <Card style={s.heroCard}>
        <LinearGradient
          colors={['rgba(224,64,251,0.12)', 'rgba(124,77,255,0.06)']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
      ...
      <View style={s.statsRow}>
        <View style={s.statItem}>
          ...
          <Text style={s.statLabel}>Edits</Text>
        </View>
        <View style={s.statDivider} />
        <View style={s.statItem}>
          <Text style={s.statValue}>38</Text>
          <Text style={s.statLabel}>Exports</Text>
        </View>
        <View style={s.statDivider} />
        <View style={s.statItem}>
          <Text style={s.statValue}>12</Text>
          <Text style={s.statLabel}>AI Tools</Text>
        </View>
      </View>
      ...
      {isPremium ? (
        <Card style={[s.planCard, { borderColor: ACCENT_BORDER }]}>
          <View style={s.planTop}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.planTitle}>GlamAI Pro Active</Text>
              <Text style={s.planSub}>{expiryDate ? `Renews ${expiryDate}` : 'Unlimited AI tools'}</Text>
            </View>
        ...
        <Pressable onPress={() => router.push('/upgrade')} style={s.upgradeCard}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.upgradeTitle}>Upgrade to GlamAI Pro</Text>
            <Text style={s.upgradeSub}>Unlock all AI tools, premium filters, and HD export.</Text>
          </View>
...
const s = StyleSheet.create({
  ...
  heroCard: { overflow: 'hidden', alignItems: 'center', gap: 5, paddingVertical: 16 },
  avatarWrap: {
  ...
  statsRow: {
    ...
    paddingVertical: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)',
  },
  statItem: { flex: 1, alignItems: 'center', gap: 2 },
  statValue: { fontSize: 18, fontWeight: '800', color: '#fff' },
  statLabel: { fontSize: 11, color: TEXT_TERTIARY, fontWeight: '500' },
  ...
  upgradeCard: {
    minHeight: 66, borderRadius: 16, overflow: 'hidden',
  ...
  sectionTitle: {
    ...
    letterSpacing: 0.8, textTransform: 'uppercase', marginTop: 3, marginBottom: -4,
  },
